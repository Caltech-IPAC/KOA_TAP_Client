...
class Archive:
    ...
    arch.query_datetime ('2018-03-16 00:00:00/2018-03-18 00:00:00', \
        outpath= './meta.xml', \
	format='ipac') 
    """
...
#            'http://koa.ipac.caltech.edu/cgi-bin/KoaAPI/nph-getCaliblist?'
    ...
    def login (self, cookiepath, **kwargs):
        ...
        cookiepath (string): a file path provided by the user to save 
                 ...
                 as input parameter for the subsequent archive 
	...
	userid     (string): a valid user id  in the KOA's user table.
        ...
        arch.login (cookiepath, userid='xxxx', password='xxxxxx'), or

        arch.login (cookiepath): and the program will prompt for 
                                 userid and password 
        """
...
#
    ...
    def print_data (self):
        ...
        if self.debug:
            logging.debug ('')
            logging.debug ('Enter archive.print_data:')
